...
실행 중 rqt_reconfigure_combo 에서 실시간 변경하면 **현재 사용 중인 파일**(보통 그 기기의 `{시리얼}/params.yaml`)에 저장되어, 기기마다 세팅이 따로 유지됩니다. 드라이버는 `NSL_PARAMS_FILE` 환경변수가 가리키는 파일을 읽고, `camera.launch.py` 가 시리얼·`calibration` 인자에 따라 자동 설정합니다.
...
- 모든 머신은 **같은 `ROS_DOMAIN_ID=42` 한 그래프**를 공유하고, Edge는 **머신 LAN IP 옥텟 네임스페이스(`cam_{옥텟}`)** 로 토픽을 분리합니다(`namespace:=auto` 가 기본값). LAN IP가 머신마다 유니크하므로 네임스페이스도 자동으로 안 겹칩니다. Host는 모든 Set의 토픽을 바로 구독할 수 있습니다.
...
| DDS 도메인 | **42 공통** (모든 머신) | — |
| **LAN IP** (`192.168.0.x`) | **머신마다 반드시 유니크** | 아래 표 |
...
| 1 Set (Edge) | `192.168.0.51` | `cam_51` |
| 2 Set (Edge) | `192.168.0.52` | `cam_52` |
| 3 Set (Edge) | `192.168.0.53` | `cam_53` |
...
3. 공장 새 카메라면 `change_camera_ip 192.168.2.220 255.255.255.0 192.168.2.1` (1-3 참고).
