...
각 Edge에서는 아래 한 줄만 실행하세요. 스크립트가 `192.168.0.N`을 읽어 namespace와 카메라 IP를 계산하고, 연결된 카메라가 `192.168.2.(N+150)`으로 저장됩니다.
...
새 터미널에서는 `source ~/colcon_ws/install/setup.bash` 를 실행하세요.
...
- **Intrinsic/Extrinsic 캘리브레이션 시에는 `calibration:=true`** 로 실행하세요.
...
**아래 순서로 진행하세요**: Intrinsic → Extrinsic
