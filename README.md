...
| `setup/check_fleet_rates.bash` | `/cam_N` 별 point cloud Hz/대역폭 비교 |
...
카메라별 실제 publish 속도는 Host/Edge 어디서든 아래처럼 비교합니다. `cam_51` 이 `cam_52` 보다 낮게 나오면 RViz/인터넷 문제가 아니라 **51번 Edge 또는 51번 센서 드라이버가 그 속도로 발행 중**이라는 뜻입니다.

```bash
bash ~/colcon_ws/src/NSL-3130AA-ROS2/setup/check_fleet_rates.bash 51 52
```
...
- **멀티뷰에서 카메라별 point cloud 업데이트 속도가 다르다** → 먼저 `setup/check_fleet_rates.bash 51 52` 로 실제 토픽 Hz를 비교하세요. 낮은 Set 쪽에서 이미 낮게 발행되면 케이블/WAN 인터넷 문제가 아니라 그 Edge의 드라이버 처리량/센서 상태입니다. Edge에서는 `use_rviz:=false use_rqt:=false` 로 띄우고, RViz는 Host의 `multiview.launch.py` 하나만 사용하세요. 그래도 낮으면 해당 Edge에서 같은 명령을 직접 실행해 로컬 Hz도 같은지 확인합니다.
