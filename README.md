...
공장 기본 IP는 `192.168.0.220`입니다. 각 Edge에서는 아래 스크립트를 권장합니다. 먼저 목표 IP(`192.168.2.220`)가 이미 살아 있는지 확인하고, USB IP write가 실패하면 카메라 전용 NIC에 임시 `192.168.0.190/24` alias를 붙여 공장 IP(`192.168.0.220`)로 Ethernet IP write를 시도합니다.

```bash
bash ~/colcon_ws/src/NSL-3130AA-ROS2/setup/setup_fleet_edge.bash --set-camera-ip
```
...
> **연결 경로(기본 `connection:=ethernet`)**: 멀티 Edge 런타임에서는 카메라 데이터가 Ethernet(`192.168.2.x`)으로만 흐릅니다. 드라이버는 `nsl_open(ip)` 전에 ping preflight를 먼저 확인해, 카메라가 안 살아 있을 때 SDK가 USB fallback을 건드리며 `libusb_get_string_descriptor_ascii error`를 반복하는 경로를 피합니다. USB는 IP write 또는 복구용 `connection:=auto`에서만 사용하세요.

> **토픽 네임스페이스(기본 동작)**: `namespace:=auto`(기본)면 **이 머신의 `192.168.0.x` IP 마지막 옥텟**을 따서 `cam_{옥텟}` 네임스페이스가 붙고, 토픽은 `/cam_{옥텟}/camera/...` 로 나옵니다(예: 이 머신이 `192.168.0.58` → `/cam_58/camera/point_cloud`). 1 Edge–1 카메라 전제라 머신 LAN IP가 곧 그 카메라의 고유 접두어가 됩니다(6장 참고). rviz 설정도 자동으로 맞춰집니다.
...
**퍼블리시 토픽:** (기본 네임스페이스 적용 시 `{ns}` = `cam_{IP옥텟}`. `namespace:=''` 면 접두어 없이 `/camera/rgb/image_raw` 등)
...
**frame_id**: 네임스페이스 기반으로 자동 결정됩니다 → `{ns}_lidar_frame` (예: `cam_58_lidar_frame`)  
...
> **네임스페이스 (자동)**: intrinsic/extrinsic 런치도 camera.launch.py와 **똑같이 `cam_{IP옥텟}` 으로 토픽 네임스페이스를 맞춥니다**(1 Edge–1 카메라 전제). 따라서 카메라를 기본값(`namespace:=auto`)으로 띄웠다면 캘리브레이션도 **추가 인자 없이** 그대로 돌리면 됩니다(예: `/cam_58/camera/...` 구독).
> - 다른 머신/카메라를 대상으로 하려면 `namespace:=cam_59` 처럼 넘기세요.
...
- **TF**: `camera.launch.py` 가 extrinsic을 읽어 `{ns}_lidar_frame → {ns}_camera_frame` 정적 TF를 publish합니다(예: `cam_58_lidar_frame → cam_58_camera_frame`). RViz의 TF 디스플레이나 아래 명령으로 확인할 수 있습니다:

```bash
ros2 run tf2_ros tf2_echo {ns}_lidar_frame {ns}_camera_frame   # 예: cam_58_lidar_frame cam_58_camera_frame
```
...
| 인자 | 기본값 | 설명 |
|---|---|---|
| `connection` | `ethernet` | 평시 런타임. USB fallback 없음 |
| `camera_ip` | `192.168.2.220` | Edge↔카메라 전용망의 카메라 IP |
| `camera_netmask` | `255.255.255.0` | `connection:=auto` 복구 시 USB로 저장할 netmask |
| `camera_gateway` | `192.168.2.1` | `connection:=auto` 복구 시 USB로 저장할 gateway |
| `net_preflight` | `true` | ping 확인 실패 시 SDK 호출을 생략해 USB fallback churn 방지 |
...
- 모든 머신은 **같은 `ROS_DOMAIN_ID=42` 한 그래프**를 공유하고, Edge는 **머신 LAN IP 옥텟 네임스페이스(`cam_{옥텟}`)** 로 토픽을 분리합니다(`namespace:=auto` 가 기본값). Edge LAN IP는 `192.168.0.50`~`192.168.0.59` 범위를 카메라용으로 예약합니다. Host는 모든 Set의 토픽을 바로 구독할 수 있습니다.

### 6-1. 충돌 방지 규칙

| 항목 | 규칙 | 예시 |
|------|------|------|
| 토픽 | 기본 `namespace:=auto` → `/cam_{옥텟}/camera/...` 자동 분리 (옥텟 = 머신 192.168.0.x) | `/cam_58/camera/point_cloud` |
| 노드 | 네임스페이스 하위로 자동 | `/cam_58/roboscan_publish_node` |
| TF frame | 네임스페이스 기반 자동 (머신 LAN IP라 유니크) | `cam_58_lidar_frame` / `cam_58_camera_frame` |
| DDS 도메인 | **42 공통** (모든 머신) | — |
| **LAN IP** (`192.168.0.x`) | Edge는 `192.168.0.50-59`, 머신마다 반드시 유니크 | 아래 표 |
| 카메라망 IP (`192.168.2.x`) | Edge마다 **독립 NIC**라 재사용 OK | 모든 Edge 카메라 `192.168.2.220`, 호스트 NIC `192.168.2.100` |

> 카메라망(`192.168.2.x`)은 Edge↔카메라 1:1 전용 링크라 사내망에 브리지하지 않는 한 Edge마다 같은 주소를 그대로 써도 충돌하지 않습니다. 충돌을 신경 쓸 곳은 **공유 LAN(`192.168.0.x`)** 뿐입니다.

**LAN IP 할당 예시** (중복만 없으면 숫자는 자유) — **LAN IP 마지막 옥텟이 곧 그 Edge의 토픽 네임스페이스**가 됩니다:

| 머신 | LAN IP | 토픽 네임스페이스 |
|------|--------|------|
| Host | `192.168.0.10` | (수신 전용) |
| Edge 0 | `192.168.0.50` | `cam_50` |
| Edge 1 | `192.168.0.51` | `cam_51` |
| ... | ... | ... |
| Edge 9 | `192.168.0.59` | `cam_59` |
...
또는 각 Edge에서 아래 스크립트로 같은 설정을 idempotent하게 넣을 수 있습니다:
...
- `namespace` 는 **기본이 `auto`** 라 아무 인자 없이도 머신 LAN IP 옥텟을 읽어 `/cam_{옥텟}/camera/...` 로 퍼블리시합니다(`192.168.0.x` 미검출 시 자동으로 네임스페이스 없이 동작).
- rviz/rqt는 Edge에선 끄고(`use_rviz:=false use_rqt:=false`), 아래처럼 Host에서 봅니다.

### 6-4. Host 에서 보기

```bash
ros2 topic list | grep camera/point_cloud                 # 살아 있는 카메라(cam_옥텟) 확인
ros2 topic hz   /cam_58/camera/point_cloud                # 수신 확인 (예시)
rviz2   # PointCloud2 디스플레이 토픽을 /cam_{옥텟}/camera/point_cloud 로 지정
```
...
1. LAN IP를 **중복 없이** 부여(6-1 표).
2. `~/.bashrc` 에 `ROS_DOMAIN_ID=42` + docker 비활성화 (6-2) — 머신마다 1회.
3. 공장 새 카메라면 `bash ~/colcon_ws/src/NSL-3130AA-ROS2/setup/setup_fleet_edge.bash --set-camera-ip` 후 카메라 전원 재인가.
4. `colcon build && source install/setup.bash`.
5. 그냥 실행만 하면(`namespace` 기본 `auto`) → `/cam_{옥텟}/camera/...` 자동 생성. 끝.

> `--set-camera-ip` 중 `libusb_get_string_descriptor_ascii error`가 나면서 실패해도, `192.168.0.220` 공장 IP가 Ethernet에서 보이면 스크립트가 자동으로 그 경로를 사용합니다. 이 fallback까지 실패하면 카메라 전원/케이블 또는 카메라 NIC 링크를 확인하세요.
